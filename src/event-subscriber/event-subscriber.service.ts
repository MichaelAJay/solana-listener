...
export class EventSubscriberService implements OnModuleInit {
  ...
  constructor(private readonly configService: ConfigService, private readonly fileLogger: FileLoggerService) {
    const connectionURL = this.configService.get<string>('SOLANA_JSON_RPC_ENDPOINT_URL');
    if (!connectionURL) {
        console.error('SOLANA_JSON_RPC_ENDPOINT_URL is not defined in the environment variables.');
        process.exit(1);
    }
    this.connection = new Connection(connectionURL, 'confirmed');
    
    const systemWalletPublicKeyString = this.configService.get<string>('SYSTEM_WALLET_PUBLIC_KEY_STRING');
    if (!systemWalletPublicKeyString) {
        console.error('SYSTEM_WALLET_PUBLIC_KEY_STRING is not defined in the environment variables.');
        process.exit(1);
    }
  ...
  async onModuleInit() {
    try {
        ...
        const accountInfo = await this.connection.getAccountInfo(this.systemPublicKey);
        if (accountInfo === null) {
            console.error('System wallet account not found on-chain. It might be a new account or on a different network. Exiting.');
            process.exit(1);
        } else {
        ...
        console.error('Failed to initialize EventSubscriberService:', err.message);
        process.exit(1);
    }
  }
